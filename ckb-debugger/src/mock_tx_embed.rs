...
/// Processes a transaction template by replacing placeholders in the provided data and returns the final processed
/// string.
pub fn mock_tx_embed(path: PathBuf, data: &str) -> String {
    let mut embed = MockTxEmbed::new(path, data.to_string());
    embed.replace_all()
}
...
impl MockTxEmbed {
    ...
    /// Replaces {{ data <path> }} placeholders with hex-encoded file content.
    ...
    /// Replaces {{ hash <path> }} placeholders with Blake2b hash of file content.
    ...
    /// Processes {{ def_type <name> }} placeholders to generate and store type ID script hashes.
    ...
    /// Replaces {{ def_type <name> }} placeholders with JSON-serialized script objects.
    ...
    /// Replaces {{ ref_type <name> }} placeholders with previously computed type ID script hashes.
