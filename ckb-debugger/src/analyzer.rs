...
pub fn analyze_cell_dep(
    ...
    if !cset.contains(&cell_dep) {
        println!("Check Fail: {} used unprovided cell dep", keyfmt(path));
        std::process::exit(1);
    }
}

pub fn analyze_header_dep(path: Vec<Key>, data: &ckb_mock_tx_types::ReprMockTransaction, header_dep: &ckb_types::H256) {
    let hset: Vec<ckb_types::H256> = data.mock_info.header_deps.iter().map(|e| e.hash.clone()).collect();
    if !hset.contains(&header_dep) {
        println!("Check Fail: {} used unprovided header dep", keyfmt(path));
        std::process::exit(1);
    }
}
...
pub fn analyze_input(
    ...
    if !iset.contains(&input) {
        println!("Check Fail: {} used unprovided input", keyfmt(path));
    }
}

pub fn analyze_output(path: Vec<Key>, data: &ckb_mock_tx_types::ReprMockTransaction) {
    if data.tx.outputs.len() != data.tx.outputs_data.len() {
        println!("Check Fail: {} outputs and outputs_data are not one-to-one correspondence", keyfmt(path));
        std::process::exit(1);
    }
}

pub fn prelude(data: &str) {
...
pub fn prelude_cell_dep(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_contains_key(path: Vec<Key>, data: &serde_json::Value, key: &str) {
    if !data.as_object().unwrap().contains_key(key) {
        println!("Check Fail: {} missing members: {}", keyfmt(path), key);
        std::process::exit(1);
    }
}

pub fn prelude_dep_type(path: Vec<Key>, data: &serde_json::Value) {
    if data.as_str().is_none() {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal dep type");
        std::process::exit(1);
    }
...
pub fn prelude_hash(path: Vec<Key>, data: &serde_json::Value) {
    prelude_hex(path.clone(), data);
    if data.as_str().unwrap().len() != 66 {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal hash");
        std::process::exit(1);
    }
}

pub fn prelude_hash_type(path: Vec<Key>, data: &serde_json::Value) {
    if data.as_str().is_none() {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal hash type");
        std::process::exit(1);
    }
...
pub fn prelude_hex(path: Vec<Key>, data: &serde_json::Value) {
    if data.as_str().is_none() {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal hex string");
        std::process::exit(1);
    }
...
pub fn prelude_input(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_mock_info(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_out_point(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_output(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_script(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_tx(path: Vec<Key>, data: &serde_json::Value) {
...
pub fn prelude_u128(path: Vec<Key>, data: &serde_json::Value) {
    if !data.is_string() || !data.as_str().unwrap().starts_with("0x") {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal u128");
        std::process::exit(1);
    }
...
pub fn prelude_u32(path: Vec<Key>, data: &serde_json::Value) {
    if !data.is_string() || !data.as_str().unwrap().starts_with("0x") {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal u32");
        std::process::exit(1);
    }
...
pub fn prelude_u64(path: Vec<Key>, data: &serde_json::Value) {
    if !data.is_string() || !data.as_str().unwrap().starts_with("0x") {
        println!("Check Fail: {} {}", keyfmt(path), "is not a legal u64");
        std::process::exit(1);
    }
...
pub fn keyfmt(key: Vec<Key>) -> String {
